...
int menu(RenderWindow & window)
{
	...
	while (isMenu)
	{
		...
		if (IntRect(100, 30, 180, 50).contains(Mouse::getPosition(window))) { menuChoicePlayer.setColor(Color::Blue); menuNum = 1; }
		if (IntRect(100, 90, 220, 50).contains(Mouse::getPosition(window))) { menuLaw.setColor(Color::Blue); menuNum = 2; }
		if (IntRect(100, 150, 120, 50).contains(Mouse::getPosition(window))) { menuExit.setColor(Color::Blue); menuNum = 3; }
